...
class ChangeTextTest {
    ...
    private val emptyText = null
    private val thirdText = "Third Test-Case"
    ...
    @Test
    fun testOpenActivityScreen() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)

        device.findObject(By.res(packageName, "userInput")).text = thirdText
        Thread.sleep(3000)
        device.findObject(By.res(packageName, "buttonActivity")).click()

        Thread.sleep(3000)
        val result = device.findObject(By.res(packageName, "text")).text
        assertEquals(result, thirdText)
    }
